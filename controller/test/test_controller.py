            ...
            resource_name: {
                ...
                "Series": {
                    ...
                    "ReactivePower": {
                        "UnitOfMeasure": "kVA.{r}",
                        "Values": [
                            10 + resource_index * 1 + hour_index * 0.1
                            for hour_index in range(epoch_number - 1, epoch_number + TIME_SERIES_LENGTH)
                        ]
                    }
...
def get_controller_messages(message_generator: GeneralMessageGenerator, epoch_number: int,
    ...
    messages = []
    topics = []
    for resource_list in list(RESOURCE_NAMES.values()):
        for resource_index, resource_name in enumerate(resource_list, start=1):
            ...
            topic = '.'.join(["ControlState.PowerSetpoint", resource_name])
...
class TestController(TestAbstractSimulationComponent):
    ...
    async def epoch_tester(self, epoch_number: int, last_status_message_id: str, message_client: RabbitmqClient,
        ...
        if epoch_number == 0:
            ...
            component_inputs = []
            expected_responds = [(self.controller_generator.get_status_ready_message(0,[manager_message.message_id]),"Status.Ready")]
        else:
